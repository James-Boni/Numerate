...
import { X, Clock, Scissors, Zap, Lock } from 'lucide-react';
...
function generateHalvingQuestion(level: number): HalvingQuestion {
  const halvingDivisors = [2, 4, 8];
  const divisorIndex = Math.min(Math.floor((level - 21) / 10), halvingDivisors.length - 1);
  const availableDivisors = halvingDivisors.slice(0, divisorIndex + 2);
  
  const divisor = availableDivisors[Math.floor(Math.random() * availableDivisors.length)];
  
  const maxQuotient = Math.min(20 + Math.floor((level - 21) / 2), 50);
  const quotient = Math.floor(Math.random() * (maxQuotient - 2)) + 2;
  
  const dividend = quotient * divisor;
  const answer = quotient;
  const text = `${dividend} ÷ ${divisor}`;
  
  const halvingSteps: string[] = [];
  let current = dividend;
  let remaining = divisor;
  
  while (remaining > 1) {
    const halved = current / 2;
    halvingSteps.push(`${current} ÷ 2 = ${halved}`);
    current = halved;
    remaining = remaining / 2;
  }
  
  const hint = `Halve ${halvingSteps.length} time${halvingSteps.length > 1 ? 's' : ''}: ${halvingSteps.join(', then ')}`;
...
export default function HalvingGame() {
  ...
  const generateNextQuestion = useCallback(() => {
    const newQuestion = generateHalvingQuestion(level);
  ...
  const endGame = useCallback(() => {
    ...
    const baseXP = correctC * 20;
    const bonusXP = Math.floor(accuracy * 70);
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full relative shadow-2xl">
            ...
            <div className="text-center space-y-4 pt-4 text-slate-600">
              <p>Halving practice unlocks at <span className="font-bold text-green-600">Level 21</span> when division is introduced.</p>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full relative shadow-2xl">
            <button
              onClick={() => setLocation('/train')}
              className="absolute right-4 top-4 w-10 h-10 flex items-center justify-center rounded-full hover:bg-slate-100"
              data-testid="button-halving-cancel"
            >
              <X size={20} className="text-slate-400" />
            </button>
            ...
            <div className="flex items-center justify-center mb-4">
              <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center">
                <Scissors size={32} className="text-green-500" />
              </div>
            </div>
            
            <h2 className="text-center text-2xl font-bold text-slate-900">Halving Practice</h2>
            
            <div className="text-center space-y-4 pt-4 text-slate-600">
              <p className="text-sm">Master the halving technique for faster division:</p>
              
              <div className="bg-green-50 rounded-2xl p-4 text-left space-y-2">
                <p className="font-medium text-green-900">Example: 48 ÷ 4</p>
                <div className="text-sm text-green-700 space-y-1">
                  <p>1. 48 ÷ 2 = 24</p>
                  <p>2. 24 ÷ 2 = <span className="font-bold">12</span></p>
                </div>
              </div>
            ...
            <Button 
              ...
              className="w-full h-12 rounded-2xl font-bold mt-6 bg-green-500 hover:bg-green-600"
              data-testid="button-halving-start"
            >
              Start Practice
            </Button>
          </div>
        ...
        <div className="fixed inset-0 bg-white flex items-center justify-center z-50">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              transition={{ duration: 0.3 }}
              className="text-9xl font-black text-green-500"
            >
              {countdownNumber}
            </motion.div>
          </AnimatePresence>
      ...
      <MobileLayout className="bg-white">
        <div className="flex-1 p-8 space-y-6 flex flex-col justify-center">
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-500">Great work on your halving skills!</p>
          </div>
          ...
          <motion.div 
            ...
            className="bg-green-50 rounded-3xl p-6 text-center space-y-1"
          >
            <span className="text-xs font-bold text-green-400 uppercase tracking-widest">Questions Correct</span>
          ...
          <Button 
            ...
            onClick={() => setLocation('/train')}
            data-testid="button-halving-continue"
          >
            Continue
          </Button>
        </div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="px-4 pt-4 pb-2 flex justify-between items-center">
          <button
            ...
            data-testid="button-halving-exit"
          >
            <X size={20} />
          </button>
              ...
              >
                ...
                {showHint && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="text-sm text-green-600 bg-green-50 px-4 py-3 rounded-xl space-y-1"
                  >
                    {question.halvingSteps.map((step, i) => (
                      <div key={i}>{step}</div>
                    ))}
                  </motion.div>
                )}
